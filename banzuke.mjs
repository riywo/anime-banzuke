...
// The data is Japanese throughout (with Latin titles mixed in), so the whole sheet is set in a
// CJK face: Source Han Sans JP VF, a variable font covering kana/kanji plus Latin, so the two
// scripts inside one title stay in the same typeface.
//   npm i @fontpkg/source-han-sans-jp-vf
const FONT_FILES = {
  [FONT_FAMILY]: "@fontpkg/source-han-sans-jp-vf/SourceHanSansJP-VF.ttf.woff2",
};
...
const MAST_H = 88; // masthead height
...
const MIN_W = 2048; // narrowest canvas (CSS px)
...
const LINE = 1.45; // CJK face: tall glyph metrics, so clipping boxes need the roomier value
...
const TYPE = {
  featured: { cap: 46, rowFill: 0.78, taper: 0.76, stretch: 1.5 },
  ranked: { cap: 40, rowFill: 0.68, taper: 0.78, stretch: 1.5 },
};
...
async function row({ item, rank, size, colW, color, stretch, last }) {
  ...
  const rankW = numbered ? Math.round(RANK_INDENT + 1.3 * rankSize) : 0;
  ...
  const gap = numbered ? RANK_GAP : PAD;
  ...
  return `<div style="flex:1;display:flex;align-items:center;min-height:0;overflow:hidden;border-left:${SPINE}px solid ${color};background:${T.bg};${bb}">
    ${numbered ? `<div style="width:${rankW}px;flex:none;padding-left:${RANK_INDENT}px;text-align:left;color:${T.muted};font-weight:${T.weight};font-size:${rankSize}px;line-height:1">${rank}</div>` : ""}
...
export async function sheet() {
  ...
  const countW =
    ...
    2 * MAST_PAD +
    BW;
  // A short Japanese title is far narrower than the masthead, so let it stretch across the
  // whole band rather than sitting in a corner of it.
